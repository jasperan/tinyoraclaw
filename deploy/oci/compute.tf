...
resource "oci_core_instance" "tinyoraclaw" {
  compartment_id      = var.compartment_ocid
  availability_domain = data.oci_identity_availability_domains.ads.availability_domains[0].name
  display_name        = "tinyoraclaw"
  ...
  create_vnic_details {
    subnet_id        = oci_core_subnet.tinyoraclaw.id
    assign_public_ip = true
    display_name     = "tinyoraclaw-vnic"
  }
  ...
  metadata = {
    ...
    user_data = base64encode(templatefile("${path.module}/cloud-init.yaml", {
      ...
      adb_dsn           = var.use_autonomous_db ? oci_database_autonomous_database.tinyoraclaw[0].connection_strings[0].all_connection_strings["LOW"] : ""
      adb_wallet_base64 = var.use_autonomous_db ? oci_database_autonomous_database_wallet.tinyoraclaw[0].content : ""
    }))
  }
